#!/usr/bin/env bash
# HoCap → Shadow Hand full pipeline
# 0. process_datasets/hocap.py  — MANO trajectory 생성
...
run_pipeline() {
    ...
    run_step gen_xml    "${task}" "${data_id}" \
        python spider/preprocess/generate_xml.py "${robot_args[@]}" --no-show-viewer --act-scene || { _skip_rest "${task}" "${data_id}"; return; }
